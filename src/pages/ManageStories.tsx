import { useState, useEffect } from 'react';
import { supabase } from '../supabase';
import { Link } from 'react-router-dom';

export default function ManageStories() {
    const [stories, setStories] = useState<any[]>([]);

    const fetchStories = async () => {
        const { data } = await supabase.from('stories').select('*, chapters(id, chapter_number, title)').order('created_at', { ascending: false });
        if (data) setStories(data);
    };

    useEffect(() => { fetchStories(); }, []);

    return (
        <div style={{ padding: '40px', background: '#F2B29A', minHeight: '100vh' }}>
            <div style={{ background: 'white', padding: '30px', maxWidth: '900px', margin: 'auto', border: '2px solid #3E2723', textAlign: 'left' }}>
                <h2 style={{ borderBottom: '2px solid #3E2723', paddingBottom: '10px' }}>VAULT MANAGEMENT</h2>
                <Link to="/archive">← Back to Archive</Link>

                {stories.map(s => (
                    <div key={s.id} style={{ border: '1px solid #eee', padding: '15px', marginTop: '20px' }}>
                        <h3>{s.title}</h3>
                        <div style={{ background: '#f9f9f9', padding: '10px' }}>
                            {s.chapters?.sort((a: any, b: any) => a.chapter_number - b.chapter_number).map((ch: any) => (
                                <div key={ch.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #eee' }}>
                                    <span>Chapter {ch.chapter_number}</span>
                                    {/* 🌟 THIS LINK GOES TO THE EDITOR */}
                                    <Link to={`/edit-chapter/${ch.id}`} style={{ fontWeight: 'bold', color: '#3E2723' }}>[EDIT CONTENT]</Link>
                                </div>
                            ))}
                        </div>
                    </div>
                ))}
            </div>
        </div>
    );
}
